...
def problem_solve(request):
    ...
    if request.method == 'POST': 
        ...
        Problem.objects.create(student = Student.objects.get(id=request.session['student_id']),divisor=int(request.POST['divisor']),dividend=int(request.POST['dividend']),student_answer=answer,is_correct=correct)
